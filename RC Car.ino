...
distance_F = Ultrasonic_read();
 Serial.print("S=");Serial.println(distance_F);
  if (distance_F < 10){backword();}
    else{     if(bt_ir_data == 1){forword(); }  // if the bt_data is '1' the DC motor will go forward
...
distance_F = Ultrasonic_read();
 Serial.print("S=");Serial.println(distance_F);
  if (distance_F < 10){backword();}
    else{ if((digitalRead(R_S) == 0)&&(digitalRead(L_S) == 0)){forword();}  //if Right Sensor and Left Sensor are at White color then it will call forword function
